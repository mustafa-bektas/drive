...
class CarEnv:
    ...
    def _calculate_reward(self, action):
        ...
        # Relative error instead of absolute difference
        ...
        # Control smoothness rewards (these work well across speeds)
        if action != self.last_action and action != 8 and self.last_action != 8:
            if abs(action - self.last_action) > 2:
                reward -= 0.2
        
        # Penalize extreme behavior relative to target speed
...
def train_with_curriculum(env, agent, curriculum_stages, episodes_per_stage=500, 
    ...
    """
    Train a DQN agent using curriculum learning with progressively more challenging speed ranges.
    
    Args:
        env: The CarEnv environment
        agent: The DQN agent
        curriculum_stages: List of dictionaries defining each curriculum stage
                          Each dict should have 'speed_min_kmh', 'speed_max_kmh', and 'target_score'
        episodes_per_stage: Maximum episodes per stage if target score not reached
...
# Define the curriculum stages (example - you can adjust these)
curriculum_stages = [
    {'speed_min_kmh': 30, 'speed_max_kmh': 40, 'target_score': 50},  # Start with low speeds
    {'speed_min_kmh': 45, 'speed_max_kmh': 55, 'target_score': 50},  # Medium speeds
    {'speed_min_kmh': 70, 'speed_max_kmh': 90, 'target_score': 50},  # Higher speeds
    {'speed_min_kmh': 20, 'speed_max_kmh': 100, 'target_score': 45}  # Full range (slightly lower target)
]
...
if __name__ == "__main__":
    ...
    # Create environment and agent as before
